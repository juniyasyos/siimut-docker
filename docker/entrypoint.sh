...
if [[ -f .env ]]; then
  ...
  DB_CONNECTION_DEFAULT="${DB_CONNECTION:-${DB_DRIVER:-pgsql}}"
  DB_HOST_DEFAULT="${DB_HOST:-db}"
  # Pilih port default berdasarkan driver
  if [[ "$DB_CONNECTION_DEFAULT" == "mysql" ]]; then
    DB_PORT_DEFAULT="${DB_PORT:-3306}"
    DB_NAME_DEFAULT="${DB_DATABASE:-${MYSQL_DATABASE:-laravel}}"
    DB_USER_DEFAULT="${DB_USERNAME:-${MYSQL_USER:-laravel}}"
    DB_PASS_DEFAULT="${DB_PASSWORD:-${MYSQL_PASSWORD:-laravel}}"
  else
